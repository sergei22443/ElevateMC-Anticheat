...
// Original check by Grim
@CheckInfo(name = "Timer", type = "A", description = "Increased game speed.")
public class TimerA extends Check {
    ...
    public void onPacketReceive(final PacketReceiveEvent event) {
        if (hasGottenMovementAfterTransaction && checkForTransaction(event.getPacketType())) {
            ...
            lastMovementPlayerClock = data.getConnectionProcessor().getPlayerClockAtLeast();
    ...
    public boolean shouldCountPacketForTimer(PacketTypeCommon packetType) {
        ...
        return WrapperPlayClientPlayerFlying.isFlying(packetType) && !data.getPositionProcessor().isTeleporting();
    }
}